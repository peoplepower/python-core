...
class TestLocations(unittest.TestCase):
    ...
    def test_get_presence(self):
        """Test Case ID: TC-Locations-001
        Title: Get Presence IDs (General)
        Priority: P1
        """
        self.mock_adapter.get.return_value = {'ibeaconUuids': ['ABCD-1234-EFGH-5678']}
        result = self.loc.get_presence()
        self.mock_adapter.get.assert_called_once()
        args, kwargs = self.mock_adapter.get.call_args
        self.assertEqual(args[0], '/espapi/cloud/json/presence')
        self.assertEqual(result, {'ibeaconUuids': ['ABCD-1234-EFGH-5678']})

    def test_authorize_presence_access(self):
        """Test Case ID: TC-Locations-002
        Title: Authorize Presence Access
        Priority: P1
        """
        self.mock_adapter.post.return_value = {'resultCode': 0, 'locationId': 456}
        params_map = {
            'ibeaconUuid': 'ABCD-1234-EFGH-5678',
            'ibeaconMajor': '12',
            'ibeaconMinor': '45'
        }
        result = self.loc.authorize_presence_access(params_map=params_map)
        self.mock_adapter.post.assert_called_once()
        args, kwargs = self.mock_adapter.post.call_args
        self.assertEqual(args[0], '/espapi/cloud/json/presence')
        self.assertEqual(kwargs['ep_json']['paramsMap'], params_map)
        self.assertEqual(result, {'resultCode': 0, 'locationId': 456})

    def test_authorize_presence_access_minimal(self):
        """Test Case ID: TC-Locations-003
        Title: Authorize Presence Access with UUID Only
        Priority: P2
        """
        self.mock_adapter.post.return_value = {'resultCode': 0, 'locationId': 456}
        params_map = {'ibeaconUuid': 'ABCD-1234-EFGH-5678'}
        result = self.loc.authorize_presence_access(params_map=params_map)
        args, kwargs = self.mock_adapter.post.call_args
        self.assertEqual(kwargs['ep_json']['paramsMap']['ibeaconUuid'], 'ABCD-1234-EFGH-5678')
        self.assertEqual(result, {'resultCode': 0, 'locationId': 456})
